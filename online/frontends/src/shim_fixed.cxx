...
extern "C" {
  ...
  char *frontend_name = (char*) "shim fixed";
...
INT begin_of_run(INT run_number, char *error)
{
  ...
  if (write_root) {
    ...
    t = new TTree("t_shfx", "Shimming Fixed Probe Data");
    t->SetAutoSave(0);
    t->SetAutoFlush(0);
    
    std::string br_name("shim_fixed");
    t->Branch(br_name.c_str(), &data, daq::shim_fixed_st_string);
  }
