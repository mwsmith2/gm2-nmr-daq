...
extern "C" {
  ...
  char *frontend_name = (char*) "run-fixed";
...
daq::run_fixed_st data; // st = short trace
daq::EventManagerTrgSeq *event_manager;
daq::SyncClient *listener;
const int nprobes = RUN_FIXED_CH;
const char *const mbank_name = (char *)"RNFX";
}
...
INT frontend_init() 
{
  ...
  if (hkey) {
    ...
    conf_file += std::string("fnal/fe_run_fixed.json");
  }
...
INT begin_of_run(INT run_number, char *error)
{
  ...
  if (write_root) {
    ...
    t = new TTree("t_rnfx", "NMR Fixed Probe Data");
    t->SetAutoSave(0);
    t->SetAutoFlush(0);
    
    std::string br_name("run_fixed");
    t->Branch(br_name.c_str(), &data, daq::run_fixed_st_string);
  }
