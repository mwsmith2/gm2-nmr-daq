...
extern "C" {
  ...
  char *frontend_name = (char*) "full wf shim platform";
  ...
  EQUIPMENT equipment[] = 
    {
      {"full-wf-shim-platform", // equipment name 
...
daq::shim_platform data; // st = short trace
...
INT begin_of_run(INT run_number, char *error)
{
  ...
  sprintf(str, "full_platform_run_%05d.root", runinfo.run_number);
  strcat(filename, str);
...
INT read_platform_event(char *pevent, INT off)
{
 ...
 auto shim_data = event_manager->GetCurrentEvent();
  ...
  if (tm.size() == 0) {
    ...
    tm.resize(NMR_FID_LN);
    wf.resize(NMR_FID_LN);

    for (int n = 0; n < NMR_FID_LN; ++n) {
      tm[n] = 0.001 * n;
    }
  }
