		...
		height)
{
	...
	const char *env_name = "ONLINEDISP";
	...
	if ( getenv(env_name) )
	{
		...
		is_name = env_value +module_file_name;
	}
	else
	{
		...
		*is >> std::ws;
		...
		if (!args.empty()) {
			...
			info.hasArgs =true;
                ...
                cout<<" macroname "<<info.macroName<<endl;
	...
	macro_path += std::string(":")+std::string(env_value)+std::string("/modules");
