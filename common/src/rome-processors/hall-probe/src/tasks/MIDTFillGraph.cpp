...
{
   TString treeFile = "data-out/hall_probe_tree_";
   ...
   fEventTree = new TTree("t_hall","Hall Probe Data");
   fEventTree->Branch("hall", &hall.volt, gm2::hall_probe_str);
}
...
void MIDTFillGraph::Event()
{
  ...
  if (IsMyGraphActive()) {
    ...
    if (gAnalyzer->GetMidasDAQ()->GetHPPFBankEntries() > 0) {
      hall.temp = gAnalyzer->GetMidasDAQ()->GetHPPFBankAt(0);
      hall.volt = gAnalyzer->GetMidasDAQ()->GetHPPFBankAt(1);
      fEventTree->Fill();
    }
  }
}
